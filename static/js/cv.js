...
let transmitToVr = false;
let mrRoomJoined = false;
let lastMrTrackCount = -1;
...
const mrVrStatus  = document.getElementById("mrVrStatus");
const chkTransmitVr = document.getElementById("chkTransmitVr");
...
function isMrCaptureMode() {
  return transmitToVr && currentPipeline === "faces";
}

function setTransmitToVr(enabled) {
  transmitToVr = Boolean(enabled);
  if (socket && socket.connected && transmitToVr && !mrRoomJoined) {
    socket.emit("join_mr");
  }
  if (!transmitToVr) {
    lastMrTrackCount = -1;
  ...
  updateMrStatus();
  log(transmitToVr
    ? "Transmissão para VR ativada (use pipeline Rostos)."
    : "Transmissão para VR desativada.");
}

function updateMrStatus() {
  if (!mrVrStatus) return;
  if (!transmitToVr) {
    mrVrStatus.textContent = "VR: transmissão desligada";
    mrVrStatus.className = "mr-vr-status";
    return;
  }
  if (currentPipeline !== "faces") {
    mrVrStatus.textContent = "VR: selecione «Detecção de Rostos»";
    mrVrStatus.className = "mr-vr-status mr-vr-warn";
    return;
  }
  mrVrStatus.textContent = lastMrTrackCount >= 0
    ? `VR: ${lastMrTrackCount} pessoa(s) na cena /arvr`
    : "VR: aguardando frames…";
  mrVrStatus.className = "mr-vr-status mr-vr-on";
}

function handleCvPayload(data) {
  waitingResult = false;
  ...
  if (data.tracks) {
    lastMrTrackCount = data.tracks.length;
    updateMrStatus();
  }
...
function sendFrame(dataUrl) {
  ...
  waitingResult = true;
  if (isMrCaptureMode()) {
    socket.emit("mr_face_frame", { image: dataUrl });
  } else {
...
function initSocket() {
  ...
  socket.on("connect", () => {
    ...
    socket.emit("join_cv");
    if (transmitToVr) socket.emit("join_mr");
  });
  ...
  socket.on("cv_result", (data) => handleCvPayload(data));

  socket.on("mr_ready", ({ message }) => {
    mrRoomJoined = true;
    log(`MR: ${message}`);
  });

  socket.on("mr_face_result", (data) => handleCvPayload(data));

  socket.on("mr_tracks_update", (data) => {
    const n = (data.tracks || []).length;
    if (transmitToVr && n !== lastMrTrackCount) {
      lastMrTrackCount = n;
      updateMrStatus();
    }
  });
...
function formatFaceRows(faces, frameSize) {
  ...
  return `
    <div class="face-coords-wrap">
      <div class="face-coords-meta">Frame: ${fw}×${fh} px · centro (cx,cy) · normalizado (nx,ny)</div>
      <table class="face-coords-table">
        <thead><tr><th>ID</th><th>Centro</th><th>Bbox</th><th>Norm.</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
}
...
function renderDetections(detections) {
  const faceDet = detections.find((d) => d.type === "faces");
  detCount.textContent = faceDet?.count ?? detections.length;
  ...
  detectionsList.innerHTML = detections.map((det) => {
    ...
    const faceTable = det.type === "faces"
      ? formatFaceRows(det.faces, det.frame_size)
      : "";
    return `
      <div class="detection-item detection-item-block">
...
document.addEventListener("DOMContentLoaded", () => {
  initSocket();
  if (chkTransmitVr) {
    chkTransmitVr.addEventListener("change", () => setTransmitToVr(chkTransmitVr.checked));
  }
  updateMrStatus();
});
